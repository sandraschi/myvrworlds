...
4. Start the server:
   ...
   uv run python -m myvrworlds.server

   # HTTP mode (for web dashboard)
   uv run uvicorn myvrworlds.server:app --port 
   ```
...
| Something else | [Open a GitHub issue](https://github.com/sandraschi/myvrworlds/issues) |
